...
function tablex.deepcompare(t1, t2, ignore_mt, eps)
    ...
    for k1, v1 in pairs(t1) do
        local v2 = t2[k1]
        if not deepcompare(v1, v2, ignore_mt, eps) then
            return false
        end
    end
    ...
    for i = 1, #t1 do
        ...
        for j = 1, #t2 do
            if not visited[j] then
                local match
                if cmp then
                    match = cmp(val, t2[j]) else match = val == t2[j]
                end
                if match then
                    gotcha = j
                    break
...
function _copy (dest, src, idest, isrc, nsrc, clean_tail)
    ...
    if dest == src then
        -- special case
        if idest > isrc and iend >= idest then
            -- overlapping ranges
...
--Functions below have been added!

--- Recursively searches in tbl for element, returning the indices as a table.
function tablex.search2(tbl, element)
    local indices = {}
...
---This is what you would give the results from tablex.search2() to.
--The first argument should be the table being accessed, the following arguments should be the indices in order.
--Example use: To access tbl.bacon[a][5].c, use "tablex.search2(tbl,unpack{"bacon",a,5,"c"})" or "tablex.search2(tbl,"bacon",a,5,"c")"
function tablex.get(...)
    return select("#", ...) >= 3 and getTblVal((...)[select(2, ...)], select(3, ...)) or (...)[select(2, ...)]
end
