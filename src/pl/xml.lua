...
function _M.parse(text_or_file, is_file, use_basic)
    ...
    if use_basic then
        ...
        if not status then
            parser = _M.basic_parse else parser = lom.parse
        end
    end
    if is_file then
...
-- @param percent a table containing attribute/value pairs
...
local function append_capture(res, tbl)
    if not empty(tbl) then
        -- no point in capturing empty tables...
        local key
        if tbl._ then
            -- if $_ was set then it is meant as the top-level key for the captured table
            key = tbl._
            tbl._ = nil
            if empty(tbl) then
                return
...
local function make_number(pat)
    if pat:find '^%d+$' then
        -- $1 etc means use this as an array location
        pat = tonumber(pat)
    end
    return pat
end
