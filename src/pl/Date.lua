...
--- Date constructor.
-- @param percent this can be either
--
...
-- @param percent a table containing one of the following keys and a value:
...
function parse_date_unsafe (s, US)
    ...
    if tz then
        ...
        if tz.h ~= 0 then
            res:add { hour = -tz.h }; corrected = true
        end
        if tz.m ~= 0 then
            res:add { min = -tz.m }; corrected = true
        end
        res.utc = true
